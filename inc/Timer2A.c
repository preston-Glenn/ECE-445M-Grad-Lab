...
void Timer2A_Handler(void){
  ...
  if(diff>(TIMER2_TAILR_R + 1 ) ){
    jitter = (diff-(TIMER2_TAILR_R + 1 ) +4)/8;  // in 0.1 usec
  }else{

    if(TIMER2_TAILR_R == NVIC_ST_RELOAD_R){
        jitter = (diff + 4 + TIMER2_TAILR_R + 1)/8;  // in 0.1 usec
    } else {
      jitter = ((TIMER2_TAILR_R + 1 ) -diff+4)/8;  // in 0.1 usec
    }
  }
