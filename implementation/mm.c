...
u64 allocate_user_page(TCBptr task, u64 uva)
{
...
int map_page(TCBptr task, u64 uva, u64 phys_page)
{
    if (!task->mm.pgd) {
...
/* copy currThread's memory space to dst */
int copy_virt_memory(TCBptr dst)
{
    for (int i = 0; i < task_up_count(currThread); i++) {
        u64 kva = allocate_user_page(dst, currThread->mm.user_pages[i].uva);
        if (kva == 0)
            return -1;
        memcpy((u64*)kva, (u64*)(currThread->mm.user_pages[i].uva), PAGE_SIZE);
    }
    return 0;
}
...
int do_data_abort(u64 far, u64 esr)
{
    ...
    if (dfsc == 0x7) {
        ...
        if (map_page(currThread, far & PAGE_MASK, page) < 0)
            return -1;
        return 0;
    }
    return -1;
