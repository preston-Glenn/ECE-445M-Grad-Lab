...
static int32_t heap[HEAP_SIZE]; // static so it is not on the stack
...
void* Heap_Malloc(int32_t desiredBytes){
  ...
  uint32_t *heapPtr = &heap[0];
  int32_t index = 0;
  int32_t value = heapPtr[index];
	int a;
  ...
  if(value == numBlocks){
    ...
    heap_stats.free -= numBlocks * sizeof(int32_t);
    heap_stats.used += numBlocks * sizeof(int32_t);
    ...
    heap_stats.free -= numBlocks * sizeof(int32_t) + 8;
    heap_stats.used += numBlocks * sizeof(int32_t);
...
void* Heap_Realloc(void* oldBlock, int32_t desiredBytes){
  ...
  int32_t index = (((uint32_t)oldBlock - (uint32_t)heap) / 4) - 1;
  int32_t oldBlockSize = heap[index];
...
int32_t Heap_Free(void* pointer){
  ...
  int32_t index = (((uint32_t)pointer - (uint32_t)heap) / 4) - 1;
  // Get the size of the block
  int32_t blockSize = heap[index];
  ...
  if((index - 1 >= 0) && heap[index - 1] < 0) {
    // merge the blocks
    int32_t newSize = heap[index - 1] + heap[index] - 2;
  ...
  if(topSize < 0) {
    // merge the blocks
    int32_t newSize = topSize + bottomValue - 2;
