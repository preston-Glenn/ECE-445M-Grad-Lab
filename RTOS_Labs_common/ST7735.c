...
void ST7735_Message(uint32_t  d, uint32_t  l, char *pt, int32_t value){
  ...
  if(d==0){
    ...
    if(value<0){
      ST7735_DrawCharS(0,l,'-',ST7735_YELLOW,ST7735_BLACK, 1);
      value = -value;         // make value positive in order to print with unsigned function
    }
    ST7735_OutUDec2(value,l);
  }else{
		
    ST7735_DrawString(0,l+8,pt,ST7735_YELLOW);
    // if value is negative, print '-'
		if(value == -1) return;

    if(value<0){
      ST7735_DrawCharS(0,l+8,'-',ST7735_YELLOW,ST7735_BLACK, 1);
      value = -value;         // make value positive in order to print with unsigned function
    }
    // TODO: not sure if the cursor is at the right position
    ST7735_OutUDec2(value,l+8);
  }
	OS_Signal(&LCDFree);
